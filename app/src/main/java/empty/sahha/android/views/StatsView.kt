...
@Composable
fun StatsView() {
    var result by remember { mutableStateOf("Select a sensor to view stats") }
            ...
            items(availableSensors) { sensor ->
                Button(
                    onClick = {
                        ...
                        result = "Loading stats for ${sensor.name}..."
                        isLoading = true
                        ...
                        try {
                            Sahha.getStats(
                                ...
                                result = if (error != null) {
                                    "Error: $error"
                                } else if (stats.isNullOrEmpty()) {
                                    "No stats found for ${sensor.name}"
                                } else {
                                    buildString {
                                        append("Stats for ${sensor.name}:\n\n")
                                        stats.forEach {
